...
async function main() {
  ...
  const ctx = (await pg.query(`
    ...
    WHERE o.slug = $1 AND u.email = $2 AND tm.meter_slug = 'lead_enriched'
    LIMIT 1
  ...
  const run = (await pg.query(`
    ...
    VALUES ($1, $2, $3, 'manual', '{"source":"linkedin"}', 'running')
    RETURNING id
  `, [ctx.org_id, ctx.user_id, ctx.tile_id])).rows[0];

  // Fetch LinkedIn profile (requires Browserbase cookie-sync or manual URL)
  const profile = composio(`execute LINKEDIN_GET_PROFILE -d '{"profile_url":"https://www.linkedin.com/in/sample-lead"}'`);
  
  // Enrich with web search
  const companyInfo = composio(`execute BRAVE_SEARCH -d '{"query":"${profile.data?.company || 'Unknown'} company info"}'`);

  const enriched = await claude.messages.create({
    model: 'claude-sonnet-4-5',
    max_tokens: 300,
    messages: [{ role: 'user', content: `Enrich this lead:
Profile: ${JSON.stringify(profile.data || {})}
Company: ${JSON.stringify(companyInfo.data || {})}
Format: "Score: X/10. Why: ... Outreach angle: ..."` }]
  });

  const text = (enriched.content[0] as any).text;
  
  await pg.query('BEGIN');
  const ev = (await pg.query(`
    INSERT INTO usage_events (org_id, tile_run_id, event_type, payload)
    VALUES ($1, $2, 'lead_enriched', $3)
    RETURNING id
  `, [ctx.org_id, run.id, { profile: profile.data?.name, enrichment: text }])).rows[0];

  const identifier = createHash('sha256').update(`lead_enriched:${ev.id}`).digest('hex');
