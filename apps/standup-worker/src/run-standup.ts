...
const TILE_SLUG = 'standup-concierge';
...
async function main() {
  ...
  const ctx = (await pg.query(`
    ...
    WHERE o.slug = $1 AND u.email = $2 AND tm.meter_slug = 'standup_summary_generated'
    LIMIT 1
  ...
  const run = (await pg.query(`
    ...
    VALUES ($1, $2, $3, 'manual', '{"period":"24h"}', 'running')
    RETURNING id
  `, [ctx.org_id, ctx.user_id, ctx.tile_id])).rows[0];

  // Fetch Slack messages (yesterday)
  const slackMsg = composio(`execute SLACK_LIST_MESSAGES -d '{"channel":"general","limit":50}'`);
  // Fetch GitHub commits/PRs
  const githubData = composio(`execute GITHUB_LIST_PULL_REQUESTS -d '{"owner":"amemecometru","repo":"llai.com","state":"all","per_page":20}'`);

  const summary = await claude.messages.create({
    model: 'claude-sonnet-4-5',
    max_tokens: 500,
    messages: [{ role: 'user', content: `Generate a standup summary from this data:
Slack: ${JSON.stringify(slackMsg.data?.slice(0,10))}
GitHub: ${JSON.stringify(githubData.data?.slice(0,10))}
Format as: "Yesterday: ... Today: ... Blockers: ..."` }]
  ...
  const ev = (await pg.query(`
    ...
    VALUES ($1, $2, 'summary_generated', $3)
    RETURNING id
  `, [ctx.org_id, run.id, { summary: text }])).rows[0];

  const identifier = createHash('sha256').update(`standup_summary_generated:${ev.id}`).digest('hex');
  ...
  await pg.query(`
    ...
    VALUES ($1,$2,$3,'standup_summary_generated',$4,
  ...
  console.log(JSON.stringify({ run_id: run.id, summary: text }, null, 2));
  await pg.end();
}
