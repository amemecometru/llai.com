...
const TILE_SLUG = 'calendar-choreographer';
...
async function main() {
  ...
  const ctx = (await pg.query(`
    ...
    WHERE o.slug = $1 AND u.email = $2 AND tm.meter_slug = 'meeting_brief_generated'
    LIMIT 1
  ...
  const events = composio(`execute GMAIL_CALENDAR_LIST_EVENTS -d '{"max_results":10}'`);
  
  for (const event of events.data || []) {
    const brief = await claude.messages.create({
      model: 'claude-sonnet-4-5',
      max_tokens: 300,
      messages: [{ role: 'user', content: `Generate a meeting brief for:
Title: ${event.summary}
Time: ${event.start?.dateTime || 'TBD'}
Attendees: ${JSON.stringify(event.attendees?.map(a => a.email) || [])}
Generate: "Brief: ... Key attendees: ... Prep: ..."` }]
    });
    ...
    const ev = (await pg.query(`
      ...
      RETURNING id
    `, [ctx.org_id, run.id, { event_id: event.id, brief: text }])).rows[0];

    const identifier = createHash('sha256').update(`meeting_brief_generated:${ev.id}`).digest('hex');
